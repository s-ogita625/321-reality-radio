...
`/admin` に、放送予定・アーカイブ・ゲスト・**MC（パーソナリティ）** の **追加／編集／削除** と
**各種リンク設定** ができる管理画面があります。データは Supabase に保存され、
保存すると公開サイトに即反映されます（コードのcommit不要）。

- **放送予定 → アーカイブ移行**: 放送予定の各項目の「▶ アーカイブへ移行」で、内容をそのままアーカイブへ移動し、アーカイブURLを設定できます。
- **画像アップロード**: サムネイル（16:9）やMC画像は管理画面から直接アップロードできます（Supabase Storage の `media` バケットに保存。公開読み取り／アップロードはログイン時のみ）。
